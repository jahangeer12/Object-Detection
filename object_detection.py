...
class YOLOApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("🦄 YOLOv8 Colorful Live Detection")
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self, text="🎯 YOLOv8 Live Detection", font=("Arial Rounded MT Bold", 28)).pack(pady=20)

        
        control_frame = ctk.CTkFrame(self)
        control_frame.pack(pady=10)

        self.scan_button = ctk.CTkButton(control_frame, text="🔍 Scan Cameras", command=self.scan_cameras)
        self.scan_button.grid(row=0, column=0, padx=10)

        self.camera_menu = ctk.CTkOptionMenu(control_frame, values=["No Cameras"], command=self.select_camera)
        self.camera_menu.grid(row=0, column=1, padx=10)

        self.model_menu = ctk.CTkOptionMenu(control_frame, values=[model.name for model in ModelType],
        ...
        self.stop_button = ctk.CTkButton(control_frame, text="🛑 Stop", command=self.stop_detection, fg_color="red")
        ...
        self.fps_label = ctk.CTkLabel(self, text="FPS: 0.00 | Device: CPU", font=("Arial", 18))
        self.fps_label.pack()
